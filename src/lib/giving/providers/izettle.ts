/* ------------------------------------------------------------------ */
/*  iZettle (Zettle by PayPal) CSV mapper                              */
/* ------------------------------------------------------------------ */
/*  Typical iZettle / Zettle export columns:                           */
/*  Receipt number, Date, Time, Type, Payment method,                 */
/*  Amount, Fee, Net, Seller, Buyer                                   */
/* ------------------------------------------------------------------ */

import type { DetectedColumns, NormalizedRow } from '../types';
import { parseMoneyToPence, parseDate } from '../importUtils';

/** iZettle rows are skipped if status indicates void/cancelled. */
const REJECTED_STATUSES = new Set(['voided', 'void', 'cancelled', 'refunded', 'reversed']);

const COL_HINTS: Record<keyof DetectedColumns, string[]> = {
  date: ['date', 'transaction date', 'payment date'],
  grossAmount: ['amount', 'total', 'gross amount', 'transaction amount'],
  feeAmount: ['fee', 'zettle fee', 'izettle fee', 'commission', 'fees'],
  netAmount: ['net', 'net amount', 'payout amount'],
  donorName: ['buyer', 'customer', 'customer name', 'name', 'donor name', 'donor'],
  reference: ['receipt number', 'receipt_number', 'reference', 'id', 'transaction id'],
  payoutReference: ['payout id', 'payout_id', 'payout reference', 'deposit id'],
  status: ['status', 'type', 'payment type', 'transaction type'],
};
...
export function mapRow(
  ...
  // Skip voided / cancelled / refunded transactions
  if (cols.status) {
